...
enum result check_arg(int num_arg, char *arg, int *stack_id, char *cmd);

int main(int argc, char const *argv[]) {
    struct stack *   stack_arr[NUM_OF_STACK] = {NULL};
    ...
    while(fgets(entry_line, MAX_LINE_SIZE, stdin)) {
        ...
        if (!strcmp(cmd, "Create_stack")) {
            ...
            stack_arr[stack_id] = (struct stack *)stack_create(max_num_of_elem,
            ...
            res = check_arg(num_arg, args[0], &stack_id, cmd);
            if (FAIL == res) {
                continue;
            }
